...
class Trainer:
    def __init__(self, model_manager, mcts_config, self_play_config, training_config):
        ...
        buffer_file = self.self_play_config.get('replay_buffer_filename', 'replay_buffer.pkl')
       ...
       # Evaluation attributes
        self.best_model_manager = None # Will hold the manager for the best performing model
    ...
    def execute_training_phase(self):
        ...
        for epoch in range(self.self_play_config['epochs_per_iter']):
             print(f"  Training Epoch {epoch+1}/{self.self_play_config['epochs_per_iter']}...")
        ...
        if batches_processed > 0:
            ...
            print(f"  Avg Loss: {avg_total_loss:.4f} (Policy: {avg_policy_loss:.4f}, Value: {avg_value_loss:.4f})")
    ...
    def evaluate_model(self):
        ...
        for i in range(num_eval_games):
            ...
            print(f"  Playing evaluation game {i+1}/{num_eval_games} (Candidate plays as P{first_player})...")
            
            outcome = self.play_one_eval_game(self.model_manager, self.best_model_manager, first_player)
        ...
        print(f"  Results: Candidate={candidate_wins}, Best={best_wins}, Draws/Errors={draws}")
        ...
        if win_rate > win_threshold:
             ...
             self.model_manager.save_checkpoint(folder=checkpoint_folder, filename=self.best_model_filename)
             # Update the best_model_manager in memory to match
             self.best_model_manager.load_checkpoint(folder=checkpoint_folder, filename=self.best_model_filename)
             print("  Best model updated.")
        else:
             print(f"  Candidate model did not pass threshold ({win_threshold:.2f}). Best model remains unchanged.")
