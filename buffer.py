    ...
    if filepath.exists():
        try:
            ...
            if not isinstance(buffer, deque):
                 ...
                 print(f"Warning: Loaded buffer maxlen ({buffer.maxlen}) differs from config ({max_size}). Adjusting.")
                 # Create new deque with correct maxlen from loaded data
                 buffer = deque(list(buffer), maxlen=max_size)

            print(f"Replay buffer loaded successfully from {filepath} ({len(buffer)} items)")
            return buffer
        except Exception as e:
