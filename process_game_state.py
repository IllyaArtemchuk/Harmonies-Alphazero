    ...
    for player in [0, 1]:
        ...
        for (q, r), stack in board.items():
            ...
            for stack_pos, tile_type in enumerate(stack):
                if stack_pos >= 3:
                    break  # Max stack height encoding = 3

                try:
                    tile_idx = TILE_TYPES.index(tile_type)
                except ValueError:
                    print(
                        f"Warning: Unknown tile type '{tile_type}' encountered in state."
                    )
...
def create_global_features(game_state):
    ...
    for i in range(NUM_PILES):
        if i < len(game_state.available_piles):  # Check if pile exists
            ...
            for tile_idx, tile_type in enumerate(TILE_TYPES):
                ...
                pile_features[feature_idx] = (
                    count / PILE_SIZE
                )  # Normalize by max possible count (3)
    ...
    if game_state.tiles_in_hand:  # Check if hand is not empty
        for tile_idx, tile_type in enumerate(TILE_TYPES):
            ...
            hand_features[tile_idx] = (
                count / PILE_SIZE
            )  # Normalize by max possible hand size (3)
    ...
    for tile_idx, tile_type in enumerate(TILE_TYPES):
        initial_count = INITIAL_BAG.get(
            tile_type, 1
        )  # Avoid division by zero if somehow missing
        if initial_count > 0:
            bag_features[tile_idx] = (
                game_state.tile_bag.get(tile_type, 0) / initial_count
            )
