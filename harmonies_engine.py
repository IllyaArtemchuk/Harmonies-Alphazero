...
if len(VALID_HEXES) != expected_count:
    raise ValueError(f"FATAL: Generated hex grid has {len(VALID_HEXES)} hexes, expected {expected_count}. Check logic.")
...
def get_water_score(length):
    ...
    if length in WATER_SCORES: return WATER_SCORES[length]
    else: return WATER_SCORES[6] + (length - 6) * 4

# --- Helper Functions (get_neighbors, bfs_shortest_path - unchanged logic, uses new VALID_HEXES) ---
def get_neighbors(coord):
    if coord not in VALID_HEXES: return []
    q, r = coord
    neighbors = set()
    for dq, dr in AXIAL_DIRECTIONS:
        nq, nr = q + dq, r + dr
        if (nq, nr) in VALID_HEXES: # Check if the potential neighbor is in our defined grid
            neighbors.add((nq, nr))
...
def bfs_shortest_path(start_node, end_node, graph_nodes, get_adj_func):
    ...
    while queue:
        ...
        for neighbor in neighbors:
            if neighbor == end_node: return distance + 1
...
class HarmoniesGameState:
    ...
    def _draw_tiles(self, num_tiles):
        ...
        for tile_type in drawn: self.tile_bag[tile_type] -= 1
        return drawn
    ...
    def _get_top_tile(self, board, coord): return board.get(coord, [None])[-1]
                ...
                for coord in VALID_HEXES:
                    ...
                    if coord not in board: # Placing on an empty spot is always allowed
                        ...
                        if tile_to_place == PLANT and top_tile == WOOD and height <= 2:
                            is_legal_placement = True
                        elif tile_to_place == STONE and top_tile == STONE and height < 3:
                            is_legal_placement = True
                        elif tile_to_place == BUILDING and top_tile in [WOOD, STONE, BUILDING] and height < 2:
                            is_legal_placement = True
    ...
    def apply_move(self, move):
        ...
        if new_state.turn_phase == "choose_pile":
            ...
            pile_index = move
            if not isinstance(pile_index, int) or not (0 <= pile_index < len(new_state.available_piles)):
            ...
            if not (isinstance(move, tuple) and len(move) == 2 and
                    isinstance(move[0], int) and isinstance(move[1], tuple)):
                raise ValueError(f"Invalid move format for placement phase: {move}. Expected (tile_index, (q, r))")

            tile_index, coord = move

            # Validate tile_index
            if not (0 <= tile_index < len(new_state.tiles_in_hand)):
                raise ValueError(f"Invalid tile_index {tile_index} for hand {new_state.tiles_in_hand}")
            # Validate coordinate
            ...
            if coord not in board:
                ...
                stack=board[coord]; top=stack[-1]; h=len(stack)
                if tile_to_place == PLANT and top == WOOD and h <= 2: is_legal=True
                elif tile_to_place == STONE and top == STONE and h < 3: is_legal=True
                elif tile_to_place == BUILDING and top in [WOOD, STONE, BUILDING] and h < 2: is_legal=True
                ...
                if is_legal:
                    ...
                    raise ValueError(f"Illegal move attempted in apply_move: Cannot place {tile_to_place} on {coord} with {stack}")

            # Advance turn phase (logic remains the same)
            if new_state.turn_phase == "place_tile_1": new_state.turn_phase = "place_tile_2"
            elif new_state.turn_phase == "place_tile_2": new_state.turn_phase = "place_tile_3"
            elif new_state.turn_phase == "place_tile_3": new_state._end_turn_actions()
    ...
    def _end_turn_actions(self):
        ...
        if end_triggered and not currently_ending:
            ...
            if player_finished == 0: # P1 triggers, P2 gets turn
                self.current_player = 1; self.turn_phase = "choose_pile"
            else: # P2 triggers or bag empty, end now
                self.turn_phase = "game_over"; self._calculate_final_scores(); self._determine_winner()
        elif currently_ending: # P2 finishes last turn
             self.turn_phase = "game_over"; self._calculate_final_scores(); self._determine_winner()
        else: # Standard turn switch
             self.current_player = 1 - self.current_player; self.turn_phase = "choose_pile"

    # --- Game End/Outcome Methods (unchanged) ---
    def is_game_over(self): return self.game_over and self.winner is not None
    def get_game_outcome(self):
        if not self.is_game_over(): return None
        if self.winner == 0: return 1
    ...
    def _determine_winner(self):
        if self.final_scores[0] > self.final_scores[1]: self.winner = 0
        elif self.final_scores[1] > self.final_scores[0]: self.winner = 1
        else: self.winner = -1
    ...
    def _score_grass(self, board):
        ...
        for coord, stack in board.items():
            ...
            if top == PLANT:
                if h == 1: score += 1
                elif h == 2 and stack[0] == WOOD: score += 3
                elif h == 3 and stack[0] == WOOD and stack[1] == WOOD: score += 7
        print("player " + str(self.current_player) + "scored " + str(score) + " points with grass!")
        return score
    ...
    def _score_mountains(self, board):
        ...
        for coord, stack in board.items():
             ...
             if top == STONE:
                 is_adj = any(self._get_top_tile(board, nc) == STONE for nc in get_neighbors(coord))
                 if is_adj:
                     if h == 1: score += 1
                     elif h == 2: score += 3
                     elif h == 3: score += 7
        print("player " + str(self.current_player) + "scored " + str(score) + " points with mountains!")
        return score
    ...
    def _score_fields(self, board):
        ...
        for start in fields:
            ...
            comp = set(); q = deque([start]); visited.add(start); comp.add(start)
            while q:
                curr = q.popleft()
                for n in get_neighbors(curr):
                    if n not in visited and self._get_top_tile(board, n) == FIELD:
                        visited.add(n); comp.add(n); q.append(n)
            if len(comp) >= 2: score += 5
        print("player " + str(self.current_player) + "scored " + str(score) + " points with fields!")
        return score
    ...
    def _score_buildings(self, board):
        ...
        for coord, stack in board.items():
            ...
            if top == BUILDING and h == 2:
                n_types = set(self._get_top_tile(board, nc) for nc in get_neighbors(coord))
                n_types.discard(None)
                if len(n_types) >= 3: score += 5
        print("player " + str(self.current_player) + "scored " + str(score) + " points with buildings!")
        return score
    ...
    def _score_water(self, board):
        ...
        for start in waters:
            if start in visited: continue
            comp_coords = set(); q_comp = deque([start]); visited.add(start); comp_coords.add(start)
            while q_comp:
                curr = q_comp.popleft()
                for n in get_neighbors(curr):
                    if n not in visited and self._get_top_tile(board, n) == WATER:
                        visited.add(n); comp_coords.add(n); q_comp.append(n)
            if len(comp_coords) >= 2:
                comp_list = list(comp_coords)
                def get_comp_neighbors(node): return [nei for nei in get_neighbors(node) if nei in comp_coords]
                diameter = 0
                for i in range(len(comp_list)):
                    node1 = comp_list[i]
                    q_bfs=deque([(node1, 0)]); visited_bfs={node1}; max_dist=0
                    while q_bfs:
                        curr_b, dist_b = q_bfs.popleft(); max_dist=max(max_dist, dist_b)
                        for neighbor_b in get_comp_neighbors(curr_b):
                             if neighbor_b not in visited_bfs:
                                 visited_bfs.add(neighbor_b); q_bfs.append((neighbor_b, dist_b + 1))
                    diameter = max(diameter, max_dist)
                score += get_water_score(diameter + 1)
        print("player " + str(self.current_player) + "scored " + str(score) + " points with water!")
        return score
    ...
    def __str__(self):
        ...
        for p in [0, 1]:
            ...
            board_repr = {str(coord): stack for coord, stack in sorted(self.player_boards[p].items())}
            s += f"  {board_repr}\n"
        s += "---------------------------------------------\n"
        return s
